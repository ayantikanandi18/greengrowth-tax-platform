...
- **Clickable vs. editable** — `DataStateBadge` + `dataStateContainerClasses`: one consistent visual language (AI-suggested / verified / locked / needs-input) reused across two screens — the field-by-field Review & Traceability screen, and a live count summary (e.g. "Verified ×3, AI suggested ×2, Locked ×1") on the return Overview's shortcut card, so a preparer can see what still needs attention without opening the review screen at all. "Locked" is clickable, not disabled — clicking Sarah's locked Filing Status field shows *why* it can't change ("Confirmed directly with client, no source document"), because a system that hides its reasoning behind a disabled button isn't actually answering "what can't be changed, and why." Approving an AI-suggested field's linked insight ("Looks right") visibly flips that field's own badge to Verified — the approval and the verified state are the same fact, not two disconnected UI elements.
...
**Real:** the Next.js app, all routing/navigation, the prioritization logic on the dashboard, the status/progress state machine, the search/filter/pagination on Rivera's 247 documents, the role-switching architecture, and the deep-linking/back-navigation trail.
